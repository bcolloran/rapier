...
impl PhysicsHooks for AdhesionHook {
    fn modify_solver_contacts(&self, context: &mut ContactModificationContext) {
        ...
        match self.kind {
            ...
            // The face of each box is 1 m wide, thus this is the same pull as `Kind::Force`
            // over one collider.
...
/// Adds a fixed slab tilted by `angle`, with a box of 1 m on each side that clings to its
/// outward face. The box is the sticky collider.
fn add_sticky_slab(
    ...
    world.insert_collider(
        ColliderBuilder::cuboid(3.0, 0.25)
            .translation(center)
            .rotation(angle)
            .friction(friction),
        Some(slab_body),
...
/// Adds a ceiling made of `tiles` colliders, with a box of 1 m on each side that hangs under its
/// middle and receives the downward load `load`. The box is the sticky collider.
fn add_tiled_ceiling(
    ...
    for k in 0..tiles {
        ...
        let x = half_width * (2 * k + 1) as Real - 3.0;
        world.insert_collider(
            ColliderBuilder::cuboid(half_width, 0.25).translation(center + Vector::new(x, 0.25)),
            Some(ceiling_body),
        );
    }

    let (body, collider) = world.insert(
        RigidBodyBuilder::dynamic().translation(center - Vector::new(0.0, 0.5)),
        ColliderBuilder::cuboid(0.5, 0.5).active_hooks(ActiveHooks::MODIFY_SOLVER_CONTACTS),
    );
    world.bodies[body].add_force(Vector::new(0.0, -load), true);
    sticky.push(collider);
}
...
pub async fn run(viewer: &mut TestbedViewer) -> anyhow::Result<()> {
    ...
    for (k, angle) in angles.into_iter().enumerate() {
        let center = Vector::new(k as Real * 5.0 - 13.0, 6.0);
     ...
     */
    for (k, tiles) in [1, 2, 4, 8].into_iter().enumerate() {
        let center = Vector::new(k as Real * 8.0 - 12.0, -3.0);
    ...
    world.insert_collider(
        ColliderBuilder::cuboid(35.0, 0.5).translation(Vector::new(0.0, -12.0)),
        Some(floor_body),
    );
    ...
    viewer.look_at(Vec2::new(0.0, 0.0), 22.0);
