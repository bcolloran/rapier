...
impl PhysicsHooks for AdhesionClimbHook {
    fn modify_solver_contacts(&self, context: &mut ContactModificationContext) {
        for cell in &self.cells {
            ...
            let capsule_is_collider1 =
                if context.collider1 == cell.capsule && context.collider2 == cell.disk {
                    true
...
fn add_cell(
    ...
    let disk = world.colliders.insert_with_parent(
    ...
    let capsule_body = world.bodies.insert(
    ...
    viewer.set_initial_collider_color(capsule, capsule_color);
...
pub async fn run(viewer: &mut TestbedViewer) -> anyhow::Result<()> {
    ...
    let capsule_area = (2.0 * CAP_RADIUS) * (2.0 * CAP_HALF_HEIGHT)
        + std::f32::consts::PI * CAP_RADIUS * CAP_RADIUS;
    ...
    let arc_length = (DISK_RADIUS + CAP_RADIUS) * (std::f32::consts::FRAC_PI_2 - theta);
    ...
    viewer.look_at(Vec2::new(0.0, 0.0), 9.0);
