...
fn add_cell(
    ...
    if squares {
        ...
        for k in 0..10 {
            ...
            let square = world.colliders.insert_with_parent(
            ...
            viewer.set_initial_collider_color(square, Color::new(shade, shade, shade, 1.0));
        }
    } else {
        let rect = world.colliders.insert_with_parent(
        ...
        viewer.set_initial_collider_color(rect, Color::new(0.56, 0.56, 0.56, 1.0));
    }
    ...
    viewer.set_initial_collider_color(capsule, color);
    if adhesion > 0.0 {
...
pub async fn run(viewer: &mut TestbedViewer) -> anyhow::Result<()> {
    ...
    let y_center =
        ((adhesions.len() as Real - 1.0) * BLOCK_DY + (modes.len() as Real - 1.0) * SUB_DY) / 2.0;
    ...
    viewer.look_at(Vec2::new(0.0, 0.0), 3.6);
