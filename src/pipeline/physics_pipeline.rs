...
mod test {
    ...
    fn manual_init_is_idempotent() {
        ...
        let rb1 = bodies.insert(RigidBodyBuilder::fixed().build());
        let co1 =
            colliders.insert_with_parent(ColliderBuilder::ball(1.0).build(), rb1, &mut bodies);

        let rb2 = bodies.insert(RigidBodyBuilder::dynamic().build());
        let co2 =
            colliders.insert_with_parent(ColliderBuilder::ball(1.0).build(), rb2, &mut bodies);
