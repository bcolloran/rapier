//! Headless A/B timing benchmark: stock [`PhysicsWorld::step`] vs
//! [`PhysicsWorld::step_collisions_last`].
//!
//! # Purpose
//!
//! The fork's game steps with `step_collisions_last`, which runs collision detection at the end
//! of the step so the contacts match the poses the game reads. This benchmark measures whether
//! that mode costs or saves time compared with stock `step`. It replaces the `--bench` mode of
//! the old testbed (used for the June 2026 comparison), which upstream 0.35 removed, and runs
//! without the GUI.
//!
//! Scenes:
//! - The ten 2D stress-test scenes of the June run, copied from `examples2d/stress_tests/`
//!   without the viewer code: Balls, Boxes, Capsules, Convex polygons, Heightfield, Pyramid,
//!   Verticals stacks, and the three "(Stress test) joint" scenes.
//! - "Adhesion tiles": rotation-locked capsules driven around tiled valleys (floor, 40° slopes,
//!   walls) by a contact-modification hook that sets `tangent_velocity` and `adhesion_budget`,
//!   the way the game moves its player.
//! - "Spawner": a box pile hit by a projectile inserted every N steps, with old projectiles
//!   removed. A collisions-last step that follows a collider insertion or removal runs an extra
//!   collision detection before the solve, which the stress scenes never trigger.
//!
//! # Method
//!
...
//! B, odd iterations B then A, so neither mode systematically runs second on a warmer cache.
...
//! feature adds a timer call around every internal stage of both modes.
//!
//! The summary reports, per scene, the mean, standard deviation, median, quartiles and IQR of
//! both modes, the percent difference of the means, and Welch's t-test (unequal variances) with
...
//!
//! ```text
//! cargo test -p rapier2d --release --test collisions_last_bench -- --ignored --nocapture
//! ```
//!
//! With the game's features:
//!
//! ```text
//! cargo test -p rapier2d --release --features enhanced-determinism,serde-serialize --test collisions_last_bench -- --ignored --nocapture
//! ```
//!
...
//!
//! - `COLLISIONS_LAST_BENCH_ITERS`: timed iterations per scene (default 500, minimum 2).
//! - `COLLISIONS_LAST_BENCH_WARMUP`: untimed warm-up steps per scene (default 20).
//! - `COLLISIONS_LAST_BENCH_SCENES`: comma-separated filter. A scene runs if its name contains
//!   one of the entries, ignoring case; for example `joint,spawner`. Default: all scenes.
//! - `COLLISIONS_LAST_BENCH_SPAWN_EVERY`: the Spawner scene inserts a projectile every this many
//!   steps (default 10, minimum 1).
//!
//! # Output
//!
//! The summary is printed to stdout and written to
//! `$CARGO_TARGET_DIR/collisions_last_benchmark_summary.txt`, or to
//! `<workspace>/target/collisions_last_benchmark_summary.txt` when `CARGO_TARGET_DIR` is unset.
//!
//! # Warning
//!
//! Run it on an idle machine. Other builds, test runs or heavy programs running at the same time
//! skew the numbers. The alternation cancels slow drifts that affect both modes equally, but not
//! the extra noise.
...
const SUMMARY_FILE_NAME: &str = "collisions_last_benchmark_summary.txt";
...
impl Knobs {
    fn from_env() -> Self {
        let scenes = std::env::var("COLLISIONS_LAST_BENCH_SCENES")
            .ok()
            .map(|filter| {
                filter
                    .split(',')
        ...
        Self {
            iters: env_usize("COLLISIONS_LAST_BENCH_ITERS", 500, 2),
            warmup: env_usize("COLLISIONS_LAST_BENCH_WARMUP", 20, 0),
            scenes,
            spawn_every: env_usize("COLLISIONS_LAST_BENCH_SPAWN_EVERY", 10, 1),
        }
    }
...
/// A benchmark scene. Built once per stepping mode, so each world gets its own hooks and callback
/// state.
struct Scene {
    world: PhysicsWorld,
...
/// The June 2026 scenes first, under their testbed names, then the fork-specific scenes.
...
/*
 * Adhesion tiles: the game's contact pattern.
 *
 * Each cell is a valley built from 1×1 tile colliders under one fixed body: a flat floor, a 40°
 * slope rising on each side, and a vertical wall above each slope. A rotation-locked capsule is
 * driven along whatever surface it touches with `tangent_velocity`, held on with two
 * `adhesion_budget` channels (feet for floors and slopes, flank for walls), and turns around
 * halfway up a wall. It keeps crossing tile seams, slope and wall corners, like a player.
 */

const ADHESION_CELLS_X: usize = 10;
const ADHESION_CELLS_Y: usize = 10;
const ADHESION_CELL_PITCH: Vector = Vector::new(18.0, 14.0);
const ADHESION_FLOOR_TILES: usize = 6;
const ADHESION_SLOPE_TILES: usize = 5;
const ADHESION_WALL_TILES: usize = 6;
const ADHESION_SLOPE_DEG: Real = 40.0;
const ADHESION_FRICTION: Real = 1.0;
const ADHESION_CAPSULE_HALF_HEIGHT: Real = 0.5;
const ADHESION_CAPSULE_RADIUS: Real = 0.4;
const ADHESION_DRIVE_SPEED: Real = 3.0;
/// Budget totals, in multiples of the capsule's weight. With μ = 1, the flank budget gives the
/// wall contact enough friction to carry the capsule upwards.
const ADHESION_FEET_WEIGHTS: Real = 1.0;
const ADHESION_FLANK_WEIGHTS: Real = 2.0;
const FEET_CHANNEL: u32 = 0;
const FLANK_CHANNEL: u32 = 1;

struct Player {
    collider: ColliderHandle,
    body: RigidBodyHandle,
    /// World x of the cell's center.
    center_x: Real,
    /// World height above which the capsule turns back towards the other side of its cell.
    turn_height: Real,
    /// Drive direction: +1 moves rightwards along the surface, -1 leftwards.
    direction: AtomicI8,
    feet_total: Real,
    flank_total: Real,
}

/// Capsule colliders carry `user_data = player index + 1`; tiles carry 0.
struct PlayerHook {
    players: Vec<Player>,
}

fn player_index(colliders: &ColliderSet, handle: ColliderHandle) -> Option<usize> {
    (colliders[handle].user_data as usize).checked_sub(1)
}

impl PhysicsHooks for PlayerHook {
    fn modify_solver_contacts(&self, context: &mut ContactModificationContext) {
        let (player, capsule_is_collider1) = match (
            player_index(context.colliders, context.collider1),
            player_index(context.colliders, context.collider2),
        ) {
            (Some(index), None) => (&self.players[index], true),
            (None, Some(index)) => (&self.players[index], false),
            // Capsule-capsule contacts stay stock.
            _ => return,
        };

        // Turn around above the turn height. The new direction depends only on the body's
        // position, so hooks running concurrently for several manifolds of one capsule agree.
        let position = context.bodies[player.body].translation();
        let direction = if position.y > player.turn_height {
            let direction = if position.x > player.center_x { -1 } else { 1 };
            player.direction.store(direction, Ordering::Relaxed);
            direction
        } else {
            player.direction.load(Ordering::Relaxed)
        };

        // `context.normal` points from collider1 towards collider2. Make it the tile surface's
        // outward normal, pointing at the capsule.
        let outward = if capsule_is_collider1 {
            -*context.normal
        } else {
            *context.normal
        };

        // Floors and slopes up to 60° use the feet budget, walls the flank budget. Contacts on
        // the capsule's head get no adhesion.
        let budget = if outward.y >= 0.5 {
            Some((FEET_CHANNEL, player.feet_total))
        } else if outward.y > -0.5 {
            Some((FLANK_CHANNEL, player.flank_total))
        } else {
            None
        };
        if let Some((channel, total)) = budget {
            *context.adhesion_budget = Some(AdhesionBudget {
                owner: player.collider,
                channel,
                total,
            });
        }

        // The outward normal rotated clockwise points rightwards along floors, up right-hand
        // slopes and walls, and down left-hand ones.
        let drive = Vector::new(outward.y, -outward.x) * (ADHESION_DRIVE_SPEED * direction as Real);
        // `tangent_velocity` is v(collider2) - v(collider1), and the tiles are fixed.
        let tangent_velocity = if capsule_is_collider1 { -drive } else { drive };
        for contact in context.solver_contacts.iter_mut() {
            contact.tangent_velocity = tangent_velocity;
        }
    }
}

fn adhesion_tiles(_: &Knobs) -> Scene {
    let mut world = PhysicsWorld::new();
    let mut players = Vec::new();

    let theta = ADHESION_SLOPE_DEG.to_radians();
    let half_floor = ADHESION_FLOOR_TILES as Real / 2.0;
    let tile = |center: Vector, angle: Real| {
        ColliderBuilder::cuboid(0.5, 0.5)
            .translation(center)
            .rotation(angle)
            .friction(ADHESION_FRICTION)
    };

    for cx in 0..ADHESION_CELLS_X {
        for cy in 0..ADHESION_CELLS_Y {
            let origin = Vector::new(cx as Real, cy as Real) * ADHESION_CELL_PITCH;
            let tiles = world.insert_body(RigidBodyBuilder::fixed().translation(origin));

            // Floor: tops at local y = 0.
            for i in 0..ADHESION_FLOOR_TILES {
                let center = Vector::new(-half_floor + 0.5 + i as Real, -0.5);
                world.insert_collider(tile(center, 0.0), Some(tiles));
            }

            let mut slope_top_y = 0.0;
            for side in [-1.0, 1.0] {
                // Slope surface: starts at the floor's end and rises outwards.
                let along = Vector::new(side * theta.cos(), theta.sin());
                let outward = Vector::new(-side * theta.sin(), theta.cos());
                let start = Vector::new(side * half_floor, 0.0);
                for k in 0..ADHESION_SLOPE_TILES {
                    let center = start + along * (k as Real + 0.5) - outward * 0.5;
                    world.insert_collider(tile(center, side * theta), Some(tiles));
                }

                // Wall: a tile column whose inner face starts at the slope's top.
                let top = start + along * ADHESION_SLOPE_TILES as Real;
                slope_top_y = top.y;
                for j in 0..ADHESION_WALL_TILES {
                    let center = Vector::new(top.x + side * 0.5, top.y + j as Real + 0.5);
                    world.insert_collider(tile(center, 0.0), Some(tiles));
                }
            }

            // The capsule starts standing on the floor's center, driven right or left.
                ...
                RigidBodyBuilder::dynamic()
                    .translation(
                        origin
                            + Vector::new(
                                0.0,
                                ADHESION_CAPSULE_HALF_HEIGHT + ADHESION_CAPSULE_RADIUS,
                            ),
                    )
                    .lock_rotations()
                    .can_sleep(false),
            ...
            let collider = world.insert_collider(
                ColliderBuilder::capsule_y(ADHESION_CAPSULE_HALF_HEIGHT, ADHESION_CAPSULE_RADIUS)
                    .friction(ADHESION_FRICTION)
                    .active_hooks(ActiveHooks::MODIFY_SOLVER_CONTACTS)
                    .user_data(index as u128 + 1),
                Some(body),
            );
            let weight = world.bodies[body].mass() * -world.gravity.y;
            players.push(Player {
                collider,
                body,
                center_x: origin.x,
                turn_height: origin.y + slope_top_y + ADHESION_WALL_TILES as Real / 2.0,
                direction: AtomicI8::new(if (cx + cy) % 2 == 0 { 1 } else { -1 }),
                feet_total: ADHESION_FEET_WEIGHTS * weight,
                flank_total: ADHESION_FLANK_WEIGHTS * weight,
            });
        }
    }
    ...
    Scene {
        world,
        hooks: Box::new(PlayerHook { players }),
        before_step: None,
        detail: None,
    }
}
...
/*
 * Spawner: collider insertions and removals during the run.
 */
...
/// One stepping mode of one scene, and what it recorded.
struct Arm {
    scene: Scene,
    mode: Mode,
    samples_ms: Vec<f64>,
...
impl Arm {
    fn new(build: SceneBuilder, knobs: &Knobs, mode: Mode) -> Self {
        ...
        if let Mode::CollisionsLast = mode {
            scene
                .world
    ...
    fn step(&mut self, step: usize, timed: bool) {
        ...
        match self.mode {
            Mode::Step => scene.world.step_with_events(hooks, &()),
            Mode::CollisionsLast => scene.world.step_collisions_last_with_events(hooks, &()),
        }
    ...
    fn assert_sane(&self, scene_name: &str) {
        let label = match self.mode {
            Mode::Step => "step()",
            Mode::CollisionsLast => "step_collisions_last()",
        };
...
#[test]
#[ignore = "timing benchmark; run manually in release with -- --ignored --nocapture"]
fn step_vs_step_collisions_last() {
    ...
    assert!(
        !selected.is_empty(),
        "COLLISIONS_LAST_BENCH_SCENES={:?} matches no scene. Scenes: {:?}",
        std::env::var("COLLISIONS_LAST_BENCH_SCENES").unwrap_or_default(),
    ...
    for &&(name, build) in &selected {
        ...
        let mut a = Arm::new(build, &knobs, Mode::Step);
        let mut b = Arm::new(build, &knobs, Mode::CollisionsLast);
...
/*
 * Statistics, ported from the fork's 0.32 testbed (`src_testbed/testbed/app.rs` on
 * `my-changes-0.32.0`).
 */
...
/*
 * Unit tests of the statistics (fast, not ignored).
 */
