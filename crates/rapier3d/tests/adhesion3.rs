//! Contact adhesion (3D): clustered multi-manifold pairs and the area-based tangential extent.
//!
...
#[test]
fn clustered_mesh_ceiling_matches_monolithic_hold_and_break_threshold() {
    ...
    for request in [Request::Pressure(w + 3.0), Request::Budget(w + 3.0)] {
        for ceiling in [Ceiling::Cuboid, Ceiling::Trimesh] {
            let (y, manifolds, clusters, extents) = hang(ceiling, request, 1.0, 240);
            ...
            let (y, ..) = hang(ceiling, request, 5.0, 240);
            assert!(
                y < -1.0,
...
#[test]
fn adhesion_force_on_a_clustered_pair_is_applied_once() {
    ...
    let w = G;
    for ceiling in [Ceiling::Cuboid, Ceiling::Trimesh] {
        let (y, ..) = hang(ceiling, Request::Force(w + 3.0), 1.0, 240);
        assert!(
            y > -0.6,
            "{ceiling:?}: box should hold a 1 N load (y = {y})"
        );
        let (y, manifolds, clusters, _) = hang(ceiling, Request::Force(w + 3.0), 5.0, 240);
        if ceiling == Ceiling::Trimesh {
...
#[test]
fn face_contact_extent_is_about_one_square_meter() {
