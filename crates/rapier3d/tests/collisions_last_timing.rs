...
//!
//! Scenes:
//! - The eleven 3D stress-test scenes, copied from `examples3d/stress_tests/` without the viewer
//!   code: Balls, Boxes, Capsules, Convex polyhedron, Heightfield, Stacks, Pyramid, Trimesh, and
//!   the three "ImpulseJoint" scenes. Nothing changes between steps, so both orders run the
//!   same stages.
...
//!
//! ```text
//! cargo test -p rapier3d --release --test collisions_last_timing -- --ignored --nocapture
//! ```
//!
...
//! `$CARGO_TARGET_DIR/collisions_last_timing_3d_summary.txt`, or to
//! `<workspace>/target/collisions_last_timing_3d_summary.txt` when `CARGO_TARGET_DIR` is unset.
//!
//! # Warning
//!
...
const SUMMARY_FILE_NAME: &str = "collisions_last_timing_3d_summary.txt";
...
/// bodies.
fn convex_polyhedron(_: &Knobs) -> Scene {
    let mut world = PhysicsWorld::new();
...
/// An 8×8×8 box pile on the ground, awake, in the projectiles' path, and an 8×4×8 box pile
/// that starts asleep behind a divider wall. From step 0, a ball is fired at the awake pile
/// every `spawn_every` steps and removed `PROJECTILE_LIFETIME_STEPS` steps later.
fn spawner(knobs: &Knobs) -> Scene {
    ...
    let _ = world.insert(
        ...
        ColliderBuilder::cuboid(60.0, 1.0, 20.0),
    );
    for (x, half_height) in [(-40.0, 20.0), (40.0, 20.0), (12.0, 10.0)] {
        let _ = world.insert(
            RigidBodyBuilder::fixed().translation(Vector::new(x, 1.0 + half_height, 0.0)),
            ColliderBuilder::cuboid(0.5, half_height, 20.0),
        );
    }

    for (x0, columns, rows, sleeping) in [(-8.0, 8, 8, false), (14.0, 8, 4, true)] {
        for i in 0..columns {
            for j in 0..rows {
                for k in 0..8 {
                    let center =
                        Vector::new(x0 + 0.5 + i as Real, 1.5 + j as Real, k as Real - 3.5);
                    let _ = world.insert(
    ...
    let before_step = move |step: usize, world: &mut PhysicsWorld| {
        ...
        if step % spawn_every == 0 {
            // Sweep the launch height and depth over the pile, one tile per shot.
            let shot = step / spawn_every;
            let height = 2.0 + (shot % 8) as Real;
            let depth = ((shot / 8) % 8) as Real - 3.5;
            let (handle, _) = world.insert(
                RigidBodyBuilder::dynamic()
                    .translation(Vector::new(-30.0, height, depth))
                    .linvel(Vector::new(25.0, 0.0, 0.0)),
...
impl Arm {
    ...
    fn assert_sane(&self, scene_name: &str) {
        ...
        for (handle, body) in self.scene.world.bodies.iter() {
            ...
            assert!(
                translation.is_finite() && body.rotation().is_finite(),
...
fn summary_header(knobs: &Knobs) -> Vec<String> {
    ...
    let mut lines = vec![
        ...
        "Method: two identical worlds per scene; even iterations step the step() world first, \
         ...
         std::time::Instant"
            .to_string(),
        format!("rapier3d features: {}", features.join(", ")),
